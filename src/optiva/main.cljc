(ns optiva.main
  "Kotodama WASM entrypoint for the Optiva clean-room actor (L4) — Clojure port.
  ...
  facts (`optiva.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "optiva")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"optiva.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "optiva-compat" :tier tier :entities entities} 200])
...
(defn start! [] :optiva-compat/ready)
